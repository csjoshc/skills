...
Generate or edit images using the built-in `image_gen` tool (primary) or `scripts/image_gen.py` CLI (explicit fallback). Default to built-in path for photos, illustrations, and mockups.

**When to use:** "generate an image", "edit this photo", "create a logo concept".

**Invocation:**
```bash
/imagegen "brief description"
```

## Decision Tree

1. **What is the intent?**
   - New creation → Mode: `generate`.
   - Modifying existing → Mode: `edit`; use `view_image` first.

2. **Is it for project use?**
   - YES → **MANDATORY** move/copy output from `$CODEX_HOME/*` to repo path.
   - NO (preview only) → Render inline; leave in temp.

3. **Does the built-in tool fail / missing features (masks)?**
   - YES → Mention CLI fallback; requires `OPENAI_API_KEY`.
   - NO → Stay on built-in tool.
...
- **Tier 1 (reversible):** Output style slightly off → refine prompt and re-run.
- **Tier 2 (conflict):** User asks for vector/SVG via `imagegen` → **STOP**, explain that `imagegen` is for bitmap; suggest direct code editing.
- **Tier 3 (security):** Generating prohibited or NSFW content → **STOP**, block and alert immediately.

## Examples (Few-Shot)

**Example 1: Generating a hero asset**
Input: "Generate a clean hero image for a coffee shop website"
Output: `image_gen` call with structured prompt; file moved to `assets/hero.png`.

**Example 2: Editing an existing UI mockup**
Input: "Change the background of this mockup to dark blue"
Output: `view_image` → `image_gen` (edit mode) with invariant preservation.
