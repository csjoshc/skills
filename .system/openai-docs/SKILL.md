...
Authoritative guidance from OpenAI developer docs. Prioritizes the `openaiDeveloperDocs` MCP server over web search for CITATIONS and model selection.

**When to use:** "OpenAI API usage", "choose a model", "GPT-5.4 upgrade guidance".
...
1. **What is the goal?**
   - General API usage → Use MCP search/fetch.
   - Model selection → **MANDATORY** Load `references/latest-model.md`.
   - GPT-5.4 Upgrade → **MANDATORY** Load `references/upgrading-to-gpt-5p4.md`.

2. **Is MCP available?**
   - YES → Search/fetch directly.
   - NO → Attempt auto-install via `codex mcp add`.

3. **Does MCP return zero results?**
   - YES → Fall back to web search (restrict to `openai.com`).
   - NO → Cite docs only.
...
- **Tier 1 (reversible):** Outdated SDK version in results — mention the snippet version, suggest checking latest.
- **Tier 2 (conflict):** MCP docs contradict a blog post — **prioritize MCP** as source of truth.
- **Tier 3 (security):** Request involves leaked or pre-release endpoints — **STOP**, block and alert immediately.

## Examples (Few-Shot)

**Example 1: Chat Completions API**
Input: "How do I use the streaming API?"
Output: MCP fetch of `/docs/api-reference/chat/create#chat-create-stream` + code snippet.

**Example 2: Model selection**
Input: "Which model is best for long context?"
Output: `references/latest-model.md` check → O1-preview or GPT-4o recommendation.
