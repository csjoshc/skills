...
Scaffolds a new plugin with `.codex-plugin/plugin.json` and optional subfolders/marketplace entries. Normalize names to lower-hyphen-case.

**When to use:** "make a plugin", "scaffold plugin", "update marketplace".

**Invocation:**
```bash
python scripts/create_basic_plugin.py <name>
```

## Decision Tree

1. **Where should it live?**
   - Repo-local → Use repo root; add to `.agents/plugins/marketplace.json`.
   - Home-local → Use `~/plugins`; add to `~/.agents/plugins/marketplace.json`.

2. **Is a marketplace entry needed?**
   - YES → **MANDATORY** Use `--with-marketplace`.
   - NO → Just scaffold folder.

3. **What components are required?**
   - Skills, Scripts, Assets? → Use optional flags (`--with-skills`, etc.).
...
- **Tier 1 (reversible):** Placeholder left in `plugin.json` — proceed, note the `[TODO]`.
- **Tier 2 (conflict):** Plugin name collision — **STOP**, ask user for new name or use `--force`.
- **Tier 3 (security):** Plugin manifest includes plaintext credentials — **STOP**, block and alert immediately.

## Examples (Few-Shot)

**Example 1: Basic repo plugin**
Input: "Create a basic plugin 'my-helper'"
Output: normalized to `my-helper`; `create_basic_plugin.py my-helper --with-marketplace`.

**Example 2: Complex system plugin**
Input: "Create a plugin 'data-audit' with skills and scripts"
Output: `create_basic_plugin.py data-audit --with-skills --with-scripts`.
