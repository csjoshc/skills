...
- **Deploy-time ACs (container / helm / k8s tickets)**: Before finalizing any ticket whose `Files:` list touches `Dockerfile`, `helm/**/*.yaml`, or a `*-entry.sh` / `start-*.sh` / `entrypoint*.sh` / `docker-entrypoint*` script, apply the Deploy-time AC conventions in the section below. `helm lint` PASS is not deploy proof. Cross-referenced with `ticket-critic` patterns 11/12/13.
